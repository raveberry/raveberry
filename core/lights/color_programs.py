...
class Alarm(ColorProgram):
    ...
    def ring_colors(self):
        return [(self.factor, 0, 0) for led in range(self.lights.ring.LED_COUNT)]

    def strip_color(self):
        return (self.factor, 0, 0)

class Adaptive(ColorProgram):
    ...
    def strip_color(self):
        red = sum(coeff * val for coeff, val in zip(self.red_coeffs, self.current_frame)) * 3 / self.frame_length
        green = sum(coeff * val for coeff, val in zip(self.green_coeffs, self.current_frame)) * 3 / self.frame_length
        blue = sum(coeff * val for coeff, val in zip(self.blue_coeffs, self.current_frame)) * 3 / self.frame_length
        red = min(1, red)
        green = min(1, green)
        blue = min(1, blue)
