...
def get_requests_session(proxy_config):
    """This method was pulled from
    https://github.com/mopidy/mopidy-spotify/blob/master/mopidy_spotify/utils.py
    The possibility for proxy configuration was dropped and a static user agent is used
    in order to remove the dependency to mopdiy."""
    full_user_agent = "Mopidy-Spotify/4.0.1 Mopidy/3.0.2 CPython/3.7.5"
...
def _trace(*args, **kwargs):
    """This line was removed to remove the utils dependency."""
...
class OAuthClient:
    def __init__(
        ...
        self._session = get_requests_session(proxy_config or {})
    ...
    def get(self, path, cache=None, *args, **kwargs):
        ...
        if result is None or "error" in result:
            logger.error(
                "Spotify Web API request failed: " f"{result.get('error','Unknown')}"
            )
            return WebResponse(None, None)
